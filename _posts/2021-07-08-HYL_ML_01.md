...
In the example of linear regression, we can ad a term $\lambda \sum \vert \vec{w} \vert ^2$. It let the functions with smaller $w$s be better, i.e. it prefer smoother functions. It also let noises have less influence over the loss function.
